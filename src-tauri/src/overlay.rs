...
/// On Windows the overlay is drawn natively (`native_overlay`), not as a
/// WebView, so there is no persistent overlay window to create here. The
/// native overlay is created on demand in `show_overlay_state` and destroyed
/// in `hide_recording_overlay`, releasing the process once recording ends.
#[cfg(target_os = "windows")]
pub fn create_recording_overlay(_app_handle: &AppHandle) {}

/// Creates the recording overlay window and keeps it hidden by default
/// (macOS / Linux only — Windows draws natively, see above).
#[cfg(not(target_os = "macos"))]
#[cfg(not(target_os = "windows"))]
...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    // Windows draws the overlay natively (no WebView); delegate to the native
    // overlay module and skip the rest of the WebView path entirely.
    #[cfg(target_os = "windows")]
    {
        crate::native_overlay::show(app_handle, state);
        return;
    }
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        // (issue #227). Hop to the main thread to keep the geometry path uniform
        // (it also keeps macOS's NSScreen access main-thread-correct).
...
pub fn emit_recording_ready(app_handle: &AppHandle) {
    // Windows native overlay has no live-transcription text channel, so the
    // frontend "recording-ready" event has no consumer — skip the main-thread
    // hop entirely.
    #[cfg(target_os = "windows")]
    {
        let _ = app_handle;
...
pub fn update_overlay_position(app_handle: &AppHandle) {
    // Windows native overlay: reposition the native window; it owns its own
    // thread/geometry and queries the cursor's monitor itself.
    #[cfg(target_os = "windows")]
    {
        crate::native_overlay::reposition(app_handle);
        return;
    }
...
pub fn hide_recording_overlay(app_handle: &AppHandle) {
    // Windows native overlay: tear it down (no WebView fade animation) and
    // release the overlay window/thread so memory drops to zero between
    // recordings.
    #[cfg(target_os = "windows")]
    {
        let _ = app_handle;
...
pub fn emit_levels(app_handle: &AppHandle, levels: &[f32]) {
    // Windows native overlay draws no live mic meter, so there is no consumer
    // for the `mic-level` event — bail out before the throttle/IPC work.
    #[cfg(target_os = "windows")]
    {
        let _ = (app_handle, levels);
        return;
    }
